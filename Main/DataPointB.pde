...
public class DataPoint {
  ...
  public void draw() {
    ...
    //  }
    //}
    //print(getCancelledFlightsCount("LAX"));
  }
  ...
  public int countFlightsFromOrigin(String originAirportCode) {
    ...
    for (String[] data : arrayData) {
      if (data[1].equals(originAirportCode)) {
        flightsFromOrigin++;
      }
    }
  ...
  public String getFlightWithLongestDistance() {
    ...
    for (String[] data : arrayData) {
      float distance = Float.parseFloat(data[10]); // assuming distance index is 10
  ...
  public int countFlightsInOneWeek(String startDate, String endDate) {
    int count = 0;
